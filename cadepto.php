...
function operacao(DeptoHelper $deptohelper, DAOBanco $banco) {
	...
	if (strcasecmp($operacao, Constantes::EDITAR) == 0 && !is_null($salvar)) {
	    ...
	    $deptohelper->alterar($banco, $camposValores, $filtro);
    ...
    try {
	    $camposValores = populaCampos();
	    $deptohelper->incluir($banco, $camposValores);
...
function populaCampos() {
	...
	$camposValores['depto'] = isset($_POST['edtdepto']) ? $_POST['edtdepto'] : $_GET['edtdepto'];
	if (isset($_SESSION[Constantes::OBJETO_USUARIO])) {
...
function mostraTemplate(DAOBanco $banco, $resultados, $mensagem = null, $codigo = null, $operacao = null) {
	...
	if (count($resultados) > 0) {
		foreach ($resultados as $deptos) {
			$sigla = $deptos->getSigla();
			$depto = $deptos->getDepto();
		}
	}
