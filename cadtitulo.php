...
function operacao(TituloHelper $titulohelper, DAOBanco $banco) {
	...
	if (strcasecmp($operacao, Constantes::EDITAR) == 0 && !is_null($salvar)) {
	    ...
	    $titulohelper->alterar($banco, $camposValores, $filtro);
    ...
    try {
	    $camposValores = populaCampos();
	    $titulohelper->incluir($banco, $camposValores);
...
function populaCampos() {
	$camposValores = array();
	$camposValores['titulo'] = isset($_POST['edtitulo']) ? $_POST['edtitulo'] : $_GET['edtitulo'];
	if (isset($_SESSION[Constantes::OBJETO_USUARIO])) {
